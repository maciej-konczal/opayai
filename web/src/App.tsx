...
export function App() {
  ...
  async function faultPurchase(type: 'wrong_item' | 'decline_payment') { if (!purchase) return; const target = type === 'decline_payment' ? purchase.id : (purchase.order?.id ?? purchase.id); await api.fault(target, type); setNotice(type === 'decline_payment' ? 'Demo: pierwsza próba BLIK zostanie odrzucona.' : 'Demo: sklep wyśle niezgodny produkt.') }
