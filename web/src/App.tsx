...
export function App() {
  ...
  async function buy(offer: Offer) {
    if (!intent) return
    try { const result = await api.purchase(intent.id, offer.sku); setPurchase({id: result.purchase_id, intent_id: intent.id, status: result.status, order_status: 'created', proposal: result.proposal}); setSheet('cart'); setNotice('Propozycja czeka na Twój podpis — agent nie może zapłacić.') } catch (error) { setNotice(String(error)) }
  }
  ...
  async function chooseBlik() { if (!purchase) return; try { const data = await api.rail(purchase.id); setPayUrl(data.pay_url); await refreshPurchase(); setSheet('payment') } catch (error) { setNotice(String(error)) } }
  ...
  return <main className="shell">
    ...
    <section className="layout">
      <aside className="chat panel"><p className="eyebrow">01 · Agent rail</p><h1>Give an agent<br/>a mandate,<br/><em>not a wallet.</em></h1><textarea value={prompt} onChange={e => setPrompt(e.target.value)} aria-label="Purchase intent"/><button className="ink-button" onClick={draft}>Utwórz mandat <span>↗</span></button><p className="notice">{notice}</p>
